...
- Overview, Settings, Clusters, Backups, Logs, and Downloads drop restating page subtitles; fleet backup KPIs stay hidden until there is history; status badges use sentence case; server cards give map art more room, a quieter stopped state, and square list rows with a straight status rail (no notch or gap).
