...
- Dense icon-only row actions (backup history Restore/Delete, cluster member Promote/Restore/Remove, Logs clear/delete, Downloads Resume/Retry/Pause/Cancel, minimized SteamCMD Cancel) use **`variant="subtle"`** so columns stay quiet; labeled destructive **Button**s stay red **filled**; workspace icon rail drops Add/Import icons (expand the list instead) (#397).
