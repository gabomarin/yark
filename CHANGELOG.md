...
- Dense icon rows follow the same hierarchy: the server card's Start is the only filled icon (Stop/Enable likewise), Update/Install and the ghost placeholders went from tinted to subtle keeping their amber/red/ok icon colour, cancel-restart is a subtle red, and the per-player Ban in RCON is a subtle red instead of a wall of filled red. Non-destructive menu items (Restore, Restart) lost their tint - only danger is red - and the two confirm modals stopped passing ariant="light" so their primary stays filled. App-wide audit: 0 tinted buttons or icons left, 13 filled buttons total (primaries and destructive), the rest secondary/tertiary.
- Buttons follow the Fluent appearance hierarchy: 37 tinted (`variant="light"`) buttons moved to the secondary `default`, a dialog's second filled-red action became a subtle red, and the wizard's "View changes" dropped its state-driven colour. The lifecycle row now has exactly one filled button (Start/Resume, `ok`) with Restart demoted to a neutral secondary control - flow control is not a warning. Opener buttons (Browse ASA catalog, Cleanup, Edit INI template, Resume schedules) are secondary.
...
- One shell surface is painted app-wide on the AppShell main: `--app-color-surface-chrome` plus the brand art (`topography` / `tek-topology`, now white and stronger), with transparent page panes instead of per-page chrome fills. The radial glow is hidden for now (`--app-shell-atmosphere: none`) because it read yellowish over chrome.
