...
- Clusters now inspect and align Cross-ARK tribute timers and upload slots across stopped members, with per-map upload/download controls kept separate (#325).
...
- OpenCodeReview now supports OpenAI Responses API providers such as GPT Luna across review, preflight, and pull request summaries via the `OCR_PRIMARY_PROTOCOL` and `OCR_FALLBACK_PROTOCOL` Actions variables.
- OpenCodeReview Responses summaries avoid unsupported sampling settings for reasoning models such as GPT Luna.
...
- **Light theme**: a second theme ships alongside dark, with its own ramp, semantic colours, elevation ladder and Mantine scales. Surfaces stay near-white and separate with a light hairline plus the elevation ladder (a visible grey step with hard borders is what makes a light theme read like an old dialog), the logo gets a brand plate so the dark-navy lockup stays readable, the dark theme is unchanged (pinned by a snapshot test), the light accent closes the white-label contrast gap that dark still documents, and `color-scheme` follows the theme so native widgets match.
  - **Two bits of duplicate noise removed**: a launch option that needs a value is now reported once - the conflict list and the error line were saying the same sentence twice - and RCON drops the Recent commands box, since Console history already lists the same commands with their responses and a rerun action.
  - **Appearance is a first-class setting**: Settings - Appearance holds the theme (dark or light), the display size and where the server workspace puts its panels, each applied instantly and remembered across restarts. The window canvas follows the saved theme, and the app follows Windows High Contrast and the Windows animation preference: with forced colours on it takes the system palette, and with animations off it stops moving.
  - **The SteamCMD progress bar no longer sits on top of the view**: the shell reserves a row for it, and the full-height views (server workspace, its Launch tab, the server form, every page scaffold) now fill the content box instead of claiming the whole viewport. Before, those views drew 44px past the row the bar occupies, so the bar landed on the last launch options and on Save changes.
  - **Consoles, the Launch panel and switches**: the SteamCMD and update-detail consoles now read as a native console - black plate, light ink, monospace - in both themes instead of turning near-white under the light one, since a console is a literal surface rather than a themed one. In the Launch panel, Extra arguments and the effective command stay pinned on screen while only the option grid scrolls. Every switch in the app now eases with the same 100 ms transition, set once in the theme.
  - **The window canvas follows the theme**: the colour behind the window before the first frame - and behind the crash screen - used to be the dark hull for everyone, so a light-shell operator saw black on startup and on every resize. It now comes from the saved theme, the main process repaints it when the theme changes, and the dark palette keeps the value it always had. The launch splash deliberately stays on the brand navy plate in both themes: it is the light-on-dark lockup, the same reason the sidebar keeps a plate.
- **Windows High Contrast**: YARK now follows forced-colours mode - system colours for surfaces, text and controls, the focus ring on `Highlight`, separation by borders instead of fills, and no brand texture - so it stays readable with a Windows high-contrast theme on.
...
- Wide desktop layouts now keep Settings and server forms at readable line lengths, empty Clusters and Backups states use compact centered surfaces, and Overview shows a single New server action when the fleet is empty.
- The ASA launch-options catalog stopped using the accent as a fill: the selected entry and the active category filter were painted with `--app-color-accent-subtle`, which reads as a saturated blue block next to the muted palette. Both now use the app's selection recipe - a neutral fill plus the accent marker (soft hairline and the 3px rail) - so the catalog matches the sidebar, the Settings nav and the Logs / server-list rows.
- Alerts read again: their fill was `--app-color-panel`, the same surface as the cards and rows they sit on, so after the palette pass an alert was little more than a 1px tone border. They now sit on a dedicated neutral surface built from the hull steps - so a tinted or warmed plate ramp cannot turn a bar into a slab - with 12% of the severity tone mixed into it, that tone also on the border and the icon, and the title left in the text tone. Hosted Resources' **Next step** guidance also moved to the top of the page: it is why the operator opens that page, and below the status card it was easy to never reach.
- Two operator-facing fixes: the sidebar's **Experimental** chip no longer squashes the icon when the sidebar is collapsed to icons (the chip is hidden there and the tooltip already carries the label), and the Backups fleet alerts now render as full-width InfoBars on the page - solid fill, hairline, left severity accent - instead of a scroll box clipped at 280px inside a card, so a single alert is finally readable at a glance.
- Surfaces move from neutral grey to a tinted navy ramp generated from one hue (hull `#05080e` through text `#e6effd`, canvas `#010306`), with the hairline step lifted 8% so cards still separate from the page, and the shell's brand texture becomes film grain instead of the hexagonal honeycomb - the motif read as a generic dashboard pattern, the grain reads as material. Both were chosen in the appearance preview and are now the shipped defaults; the accent is untouched, so the documented white-on-accent gap stays exactly where it was. The launch splash and the error screen follow the same hull, so neither flashes the old colour on startup.
- The lifecycle **Start/Resume** uses the accent like every other primary instead of a green fill: a green CTA was the only primary outside the accent and could not follow an accent swap, and green stays a state colour (words, badges, dots). It inherits the documented white-label-on-accent gap the other filled primaries already share, so closing that is one global accent decision. The server card's remaining icon actions became bordered `default` so they read as controls, with meaning kept in the glyph (amber Update, red Cancel) because the bordered variant ignores the ActionIcon colour.
- Dense icon rows follow the same hierarchy: the server card's Start is the only filled icon (Stop/Enable likewise), Update/Install and the ghost placeholders went from tinted to subtle keeping their amber/red/ok icon colour, cancel-restart is a subtle red, and the per-player Ban in RCON is a subtle red instead of a wall of filled red. Non-destructive menu items (Restore, Restart) lost their tint - only danger is red - and the two confirm modals stopped passing variant="light" so their primary stays filled. App-wide audit: 0 tinted buttons or icons left, 13 filled buttons total (primaries and destructive), the rest secondary/tertiary.
- Buttons follow the Fluent appearance hierarchy: 37 tinted (`variant="light"`) buttons moved to the secondary `default`, a dialog's second filled-red action became a subtle red, and the wizard's "View changes" dropped its state-driven colour. The lifecycle row now has exactly one filled button (Start/Resume, accent) with Restart demoted to a neutral secondary control - flow control is not a warning. Opener buttons (Browse ASA catalog, Cleanup, Edit INI template, Resume schedules) are secondary.
- Buttons, alerts, menus, toasts and notifications now speak one colour language: 98 call sites that used Mantine's green/teal/yellow/orange were swept to the semantic palette (ok / attention), and the Alert resolver gained a success tone, so a green alert finally shows a green hairline instead of falling back to the default blue. The wizard's progression and difficulty ramps keep their distinct hues - those encode tiers, not status.
- Badges and chips read as one system: the theme gives every Badge Fluent 2 type (12px semibold on a 20px pill, 4px corners, sentence case - Mantine shipped 9-11px micro-text) and a neutral default (`light` + `gray`), so a fact or a tag stops wearing a colour. The 67 existing badges were swept by role: neutral facts and tags lost their blue, `teal`/`green` became `ok`, `yellow`/`orange`/`fossil` became `attention`, per-call-site `size`/`tt` props are gone, and the inline colour-mix badge was replaced by the `attention` token.
...
- **Hosted Resources** joins the chrome tool-pane grammar (`PageScaffold edgeToEdge`, title and Refresh inside the shell) instead of a floating page header above cards.
- **Overview** moves onto the same full-height shell as the other sidebar pages (title and actions inside, square server rows, square **Recent activity** panel) with the shell surface, completing the app-wide tool-pane grammar.
- One shell surface is painted app-wide on the AppShell main: `--app-color-surface-chrome` plus the brand honeycomb, with transparent page panes instead of per-page chrome fills. The pattern is drawn inline (the `topography` / `tek-topology` SVG assets are gone) and faded from the content's top-left with a radial mask, so the texture sits behind the page instead of tiling the whole shell. The radial glow is hidden for now (`--app-shell-atmosphere: none`) because it read yellowish over chrome.
- Overview's fleet metrics read as instrument readouts: one labelled strip with tabular display values instead of five separate cards with prose headings.
- Tables use the app's row surface instead of the library default: `YarkDataTable` now sets `--mantine-datatable-background-color` / row border / hover / selection from palette tokens, so table rows match the Overview, Logs, and Settings row recipe (solid panel, square, hairline) instead of painting the page canvas inside the pane. Table headers are `--app-color-panel-raised`. The Mods table keeps its own transparent viewport surface.
- Border and chip consistency: every translucent `color-mix(…, transparent)` border in Settings, Hosted Resources, Clusters, and the Launch options surfaces is now a solid `--app-color-border-subtle` hairline or a solid semantic tone (matching the Alert grammar); filter and tone chips drop the `999px` pill for the control radius; the Launch options rows lose their fake translucent-white fill and the selected row's doubled border (border + inset ring) for an accent border plus the same 3px left accent bar used by the download rows.
- Fluent 2 component chrome: the remaining page-level and dialog-content boxes are square (Download rows, RCON rows, Logs update rows, Settings cache/auto-start/privacy boxes, mods thumbnails excluded, map-search and cluster dialogs, the Copy-configuration wizard, the SteamCmd dock uses the dialog radius), and the decorative gradients on the download rows and the Launch options dialog are gone (Fluent 2 is flat). Pills, status dots, nav rows, and icon tiles keep their radius.
- Hosted Resources, Backups, and Clusters pages finish the shell grammar: their page-level cards are square (`radius={0}`) and every translucent panel wash (`color-mix(…, transparent)`) became a solid `--app-color-panel` with `--app-color-border-subtle` hairlines, so those surfaces no longer depend on the brand art behind them. Modal interiors keep the dialog radius.
- Accent is now a role set instead of a loose blue: `accentPalette` in `shared/theme/tokens.ts` is the single source that feeds `--ark-blue-*`, Mantine's `blue` scale, and the new `--app-color-accent-hover / -accent-text / -accent-subtle / -accent-subtle-hover / -accent-contrast` roles. All 25 `:focus-visible` rings shared six different colors and now use one `--app-color-focus-ring`. Section eyebrows, the Launch preview's **YARK-owned** category, and interactive hovers moved off `--app-color-cryo` onto the accent; `--app-color-info` keeps the server-reported informational state separate. A contrast test now covers the accent (solid >= 3:1, text >= 4.5:1 on every surface).
- Palette hygiene: `--app-color-muted` derives from the neutral ramp instead of a fixed gray; `--app-color-danger-bright` is now AA-readable (5.0:1) and every text/icon that used the `bad` fill tone moved to it; the Mantine `default` variant set (outlined buttons, badges, popovers) is mapped to palette tokens instead of Mantine's hardcoded white label; the last hardcoded `#000` / `#fff` mixes and `var(--app-*, fallback)` guards are gone. Two new gates enforce this: a scan that fails on undefined or fallback-guarded `--app-*` tokens, and a contrast test for text, muted, and semantic colors.
...
- Switches now move on the compositor and paint immediately through expensive updates, removing delayed or uneven toggles in Mods, both INI editors, Launch, and Backups.
...
- Closing the server console window (or End task / Ctrl+C) while the server is starting or running no longer shows a crash state or Windows crash toast; the card and Logs record a **Closed by user** warning instead (#524).
...
[0.15.0]: #0150---2026-08-22
[0.14.1]: #0141---2026-08-19
[0.14.0]: #0140---2026-08-19
[0.13.1]: #0131---2026-08-17
[0.13.0]: #0130---2026-08-17
[0.12.0]: #0120---2026-08-13
[0.11.0]: #0110---2026-08-12
[0.10.0]: #0100---2026-08-12
[0.9.1]: #091---2026-08-10
[0.9.0]: #090---2026-08-10
[0.8.1]: #081---2026-08-09
[0.8.0]: #080---2026-08-09
[0.7.0]: #070---2026-08-07
