...
function profile(partial: Pick<ServerProfile, "id" | "updatedAt"> & Partial<ServerProfile>): ServerProfile {
  return {
    name: "Server",
    map: "TheIsland_WP",
    installDir: "C:\\ARK",
...
describe("reconcileServerList", () => {
  it("returns the previous array when id/updatedAt sequence matches", () => {
    const previous = [profile({ id: "a", updatedAt: "t1" }), profile({ id: "b", updatedAt: "t1" })];
    const next = [profile({ id: "a", updatedAt: "t1", name: "Different ref" }), profile({ id: "b", updatedAt: "t1" })];
  ...
  it("reuses unchanged profiles when another server updates", () => {
    ...
    const next = [profile({ id: "a", updatedAt: "t1" }), profile({ id: "b", updatedAt: "t2", name: "Renamed" })];
