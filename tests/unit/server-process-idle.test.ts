...
import { clusterProcessBusyReason, isServerProcessLive } from "@shared/server/server-process-idle";
...
describe("server-process-idle", () => {
  it("treats transitional statuses as live when processLive is false", () => {
    for (const status of ["starting", "running", "stopping"] as const) {
      ...
      expect(clusterProcessBusyReason({ status, processLive: false })).toBe("Server must not be running");
    }
  });
  ...
  it("blocks error when the child is still live", () => {
    ...
    expect(clusterProcessBusyReason({ status: "error", processLive: true })).toBe("Server must not be running");
  });
