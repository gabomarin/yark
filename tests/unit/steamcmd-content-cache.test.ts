...
describe("steamcmd-content-cache", () => {
  it("resolves home and cache paths next to steamcmd.exe", () => {
    expect(resolveSteamCmdHome("C:\\tools\\steamcmd\\steamcmd.exe")).toBe("C:\\tools\\steamcmd");
    expect(resolveDepotCacheDir("C:\\tools\\steamcmd")).toBe("C:\\tools\\steamcmd\\steamapps\\depotcache");
    expect(resolveAsaContentCacheDir("C:\\tools\\steamcmd")).toBe("C:\\tools\\steamcmd\\asa_content_cache");
    expect(resolveSteamCmdCacheDir("C:\\tools\\steamcmd", "depot")).toBe("C:\\tools\\steamcmd\\steamapps\\depotcache");
    expect(resolveSteamCmdCacheDir("C:\\tools\\steamcmd", "content")).toBe("C:\\tools\\steamcmd\\asa_content_cache");
  });
  ...
  it("uses a moderate robocopy thread count for sync", async () => {
    const { ASA_CONTENT_SYNC_ROBOCOPY_THREADS } = await import("@backend/domains/updates/steamcmd-content-cache");
  ...
  it("skips content sync only when cache and install are the same path", async () => {
    ...
    const { canSkipAsaContentSync, readAsaManifestBuildId } =
...
describe("OperationCancelledError", () => {
  it("is detectable via helper", async () => {
    const { isOperationCancelledError, OperationCancelledError } =
      await import("@backend/domains/updates/steamcmd-content-cache");
