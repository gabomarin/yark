...
import { bundledSteamCmdUnusedNote, isPathUnderParent } from "@features/settings/settingsModel";
...
describe("bundledSteamCmdUnusedNote", () => {
  ...
  it("is silent when YARK is using the bundled exe", () => {
    expect(bundledSteamCmdUnusedNote(bundled, `${bundled}\\steamcmd.exe`)).toBeNull();
  });

  it("notes when a chosen SteamCMD lives elsewhere", () => {
    expect(bundledSteamCmdUnusedNote(bundled, "C:\\tools\\steamcmd\\steamcmd.exe")).toBe(
      "Not in use. YARK is using the SteamCMD you chose in Settings → SteamCMD.",
    );
  });

  it("notes when SteamCMD is not configured yet", () => {
    expect(bundledSteamCmdUnusedNote(bundled, null)).toBe("Empty until you use Install SteamCMD.");
  });
