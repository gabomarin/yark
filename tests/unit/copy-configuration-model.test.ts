...
const categories = [
  ...
  },
  {
    id: "dinos",
    label: "Dinosaurs",
    keys: [{ section: "ServerSettings", key: "TamingSpeedMultiplier" }],
  },
  {
    id: "general",
    label: "General",
...
describe("copyConfigurationModel INI selection", () => {
  it("file checkbox selects every key", () => {
    const next = toggleIniEntireFile(emptyIniFileSelection("merge"), true, categories);
  ...
  it("UI category checkbox selects only that category", () => {
    let file = toggleIniCategoryKeys(emptyIniFileSelection("merge"), categories, "rates", true);
    // Rates is only one of several ServerSettings keys → store as key refs.
    expect(file.sections).toEqual([]);
    expect(file.keys).toEqual([{ section: "ServerSettings", key: "XPMultiplier" }]);
    expect(selectedIniKeyIds(file, categories).size).toBe(1);
    expect(fileSelectionState(file, categories).indeterminate).toBe(true);
    expect(categorySelectionState(file, categories, "rates").checked).toBe(true);
    ...
    file = toggleIniKey(file, categories, "ServerSettings", "XPMultiplier", false);
    expect(selectedIniKeyIds(file, categories).size).toBe(1);
    expect(file.keys).toEqual([{ section: "ServerSettings", key: "TamingSpeedMultiplier" }]);
  });
});
...
describe("copyConfigurationModel target multi-select", () => {
  it("toggles individual and select-all targets", () => {
    ...
    expect(toggleAllTargetIds(["a"], ["b", "c"], true)).toEqual(["a", "b", "c"]);
    expect(toggleAllTargetIds(["a", "b", "c"], ["b", "c"], false)).toEqual(["a"]);
