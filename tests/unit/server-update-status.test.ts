...
function installation(overrides: Partial<ServerInstallationInfo> = {}): ServerInstallationInfo {
...
describe("getServerUpdateState", () => {
  ...
  it("marks update only when the local Steam build is behind official", () => {
    expect(getServerUpdateState(installation({ steamBuild: "build 24300000" }), "build 24346423")).toBe("available");
  });

  it("treats a local Steam build ahead of the official probe as current (#490)", () => {
    expect(getServerUpdateState(installation({ steamBuild: "build 25000000" }), "build 24346423")).toBe("current");
  });

  it("does not invent a state when a comparable build is missing", () => {
    expect(getServerUpdateState(installation(), null)).toBe("unknown");
    expect(getServerUpdateState(installation({ installed: false }), "build 24346423")).toBe("unknown");
  });
});
