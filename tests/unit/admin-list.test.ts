...
describe("admin-list", () => {
  ...
  it("resolves wiki AllowedCheaterAccountIDs.txt under Saved", () => {
    expect(adminListPath("C:\\ARK\\Island")).toBe("C:\\ARK\\Island\\ShooterGame\\Saved\\AllowedCheaterAccountIDs.txt");
  });

  it("resolves legacy Win64 AdminList.txt next to the binary", () => {
    expect(legacyAdminListPath("C:\\ARK\\Island")).toBe("C:\\ARK\\Island\\ShooterGame\\Binaries\\Win64\\AdminList.txt");
  });

  it("parses one id per line and skips comments", () => {
    expect(
      parseAdminListIds("# comment\n0002e03af5f4487985e94c6ba4080369\n\n0002aabbccddeeff0011223344556677\n"),
    ).toEqual(["0002e03af5f4487985e94c6ba4080369", "0002aabbccddeeff0011223344556677"]);
  });
  ...
  it("classifies blank / N/A / file:// as local and http(s) as remote", () => {
    ...
    expect(classifyAdminListUrl('"file://C:\\\\ARK\\\\ShooterGame\\\\Saved\\\\AllowedCheaterAccountIDs.txt"')).toBe(
      "local",
    );
  ...
  it("unwraps quoted INI URLs", () => {
    expect(unwrapIniUrl('"https://example.com/a.txt"')).toBe("https://example.com/a.txt");
  });

  it("formats http(s) AdminListURL with quotes for INI", () => {
    expect(formatAdminListUrlForIni("https://example.com/a.txt")).toBe('"https://example.com/a.txt"');
  ...
  it("classifies loopback http AdminListURL as loopback (kept verbatim)", () => {
    expect(classifyAdminListUrl("http://127.0.0.1:34567/c4537c5632d684ee.txt")).toBe("loopback");
    expect(classifyAdminListUrl('"http://localhost:9/aaaaaaaaaaaaaaaa.txt"')).toBe("loopback");
  });

  it("mirrors ASA pointer under configured userData admin-lists when wiki path has spaces", () => {
    const install = "C:\\yark_test_servers\\first test";
    const mirrorRoot = "C:\\Users\\op\\AppData\\Roaming\\yark-server-manager\\admin-lists";
    const pointer = adminListAsaPointerPath(install, mirrorRoot);
    expect(pointer.startsWith(mirrorRoot + "\\") || pointer.startsWith(mirrorRoot + "/")).toBe(true);
    ...
    expect(windowsPathFromAdminListFileUrl(formatLocalAdminListFileUrlForIni(install, mirrorRoot))).toBe(pointer);
  });
  ...
  it("syncs wiki ids into the space-free ASA pointer file", async () => {
    ...
    try {
      ...
      await writeFile(adminListPath(install), "0002e03af5f4487985e94c6ba4080369\n", "utf8");
      ...
      expect(await readFile(pointer, "utf8")).toContain("0002e03af5f4487985e94c6ba4080369");
    } finally {
  ...
  it("refresh before start re-copies out-of-band wiki edits into the mirror", async () => {
    ...
    const mirrorRoot = join(root, "admin-lists");
    try {
      await mkdir(join(install, "ShooterGame", "Saved", "Config", "WindowsServer"), { recursive: true });
      await writeFile(adminListPath(install), "0002e03af5f4487985e94c6ba4080369\n", "utf8");
      await writeFile(
      ...
      await writeFile(adminListPath(install), "0002aabbccddeeff0011223344556677\n", "utf8");
      await refreshAdminListBeforeStart(install, mirrorRoot);
      expect(await readFile(pointer, "utf8")).toBe("0002aabbccddeeff0011223344556677\n");
    } finally {
      await rm(root, { recursive: true, force: true });
    }
  });
  ...
  it("heals file:// with spaces to the space-free mirror via refresh helper", async () => {
    ...
    try {
      ...
      await writeFile(gameUserSettingsIniPath(install), `[ServerSettings]\nAdminListURL=${spacedUrl}\n`, "utf8");
      await refreshAdminListBeforeStart(install, mirrorRoot);
      const gus = await readFile(gameUserSettingsIniPath(install), "utf8");
      const written = readIniServerSetting(gus, "AdminListURL");
      expect(written).toBe(formatLocalAdminListFileUrlForIni(install, mirrorRoot));
  ...
  it("heals two-slash file:// to three-slash file:/// via refresh helper", async () => {
    ...
    try {
      ...
      const written = readIniServerSetting(await readFile(gameUserSettingsIniPath(install), "utf8"), "AdminListURL");
  ...
  it("clamps UpdateAllowedCheatersInterval (min 3, default 600)", () => {
    ...
    expect(clampUpdateAllowedCheatersInterval(Number.NaN)).toBe(DEFAULT_UPDATE_ALLOWED_CHEATERS_INTERVAL);
  });
  ...
  it("reads local ids from wiki file when AdminListURL is blank", async () => {
    ...
    try {
      ...
      await writeFile(adminListPath(root), "0002e03af5f4487985e94c6ba4080369\n", "utf8");
      await writeFile(
      ...
      expect(state.entries).toEqual([{ id: "0002e03af5f4487985e94c6ba4080369", name: null }]);
  ...
  it("writes remote config and rejects bare paths", async () => {
    ...
    try {
      ...
      expect(state.entries).toEqual([{ id: "0002aaaaaaaaaaaaaaaaaaaaaaaaaaaa", name: null }]);

      const gus = await readFile(gameUserSettingsIniPath(root), "utf8");
      expect(readIniServerSetting(gus, "AdminListURL")).toBe('"https://example.com/admins.txt"');
      expect(readIniServerSetting(gus, "UpdateAllowedCheatersInterval")).toBe("3");
    } finally {
      await rm(root, { recursive: true, force: true });
    }
  });
  ...
  it("clearing URL removes AdminListURL (optional; no file:// rewrite)", async () => {
    ...
    try {
      ...
      expect(readIniServerSetting(gus, "UpdateAllowedCheatersInterval")).toBe("600");
    } finally {
      await rm(root, { recursive: true, force: true });
    }
  });
  ...
  it("heals legacy blank/N/A local AdminListURL to file:/// via refresh helper", async () => {
    ...
    try {
      ...
      expect(state.entries).toEqual([{ id: "0002e03af5f4487985e94c6ba4080369", name: null }]);
      const gus = await readFile(gameUserSettingsIniPath(root), "utf8");
      expect(readIniServerSetting(gus, "AdminListURL")).toBe(formatLocalAdminListFileUrlForIni(root));
    } finally {
      await rm(root, { recursive: true, force: true });
    }
  });
  ...
  it("does not seed empty wiki from names sidecar on read", async () => {
    ...
    try {
      ...
      await writeFile(gameUserSettingsIniPath(root), "[ServerSettings]\nAdminListURL=N/A\n", "utf8");
      await learnAdminListNames(root, [{ id: "0002e03af5f4487985e94c6ba4080369", name: "gabomarin26" }]);
      expect(existsSync(adminListNamesPath(root))).toBe(true);
      const state = await getState(root);
      expect(state.mode).toBe("local");
      expect(state.entries).toEqual([]);
