  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (uses.startsWith("docker://")) {
      out.push({
        ...
        line: i + 1,
        uses: usesRaw,
        reason: "docker:// Actions must not use mutable tags; prefer a digest or avoid docker Actions",
      });
      continue;
    }
    ...
    if (!VERSION_COMMENT.test(inlineComment)) {
      out.push({
        ...
        line: i + 1,
        uses: usesRaw,
        reason: "missing same-line version comment (use `# vX.Y.Z` after the SHA so Dependabot can update the pin)",
      });
    }
  }
...
function lintActionsPins() {
  ...
  console.error("Pin each Action as `uses: owner/name@<40-char-sha> # vX.Y.Z`. See docs/github-actions.md.");
  return 1;
}
