...
function main() {
  ...
  const violations = [
    ...collectSizeViolations(FEATURES_DIR, feature.allowed, feature.growthSlack, (abs) =>
      abs.endsWith(".tsx") ? feature.maxTsxLines : feature.maxTsLines,
    ),
    ...collectSizeViolations(BACKEND_DIR, backend.allowed, backend.growthSlack, () => backend.maxTsLines),
  ];
