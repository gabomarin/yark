...
async function run() {
  ...
  try {
    ...
    const bulkSwitch = page.getByRole("switch", { name: /^(Disable|Enable) /i }).first();
    if (!(await bulkSwitch.isChecked())) {
    ...
    await page.getByText(DEMO_MOD_ID, { exact: true }).first().waitFor({ state: "detached", timeout: 15000 });
