...
- [-] Vista de logs de proceso / update / backup desde la UI (filtros de eventos, búsqueda, scroll interno, copia y exportación listos; rediseño Fase 2 aplicado: tabs superiores Events/Runtime/Update Logs/Backups, panel "Update History" paginado con filtro por estado y panel "Update Details" con grid Date/Duration/Size/Type, sub-tabs Log Output/Files y botón "Open in external viewer" vía `shell.openPath`; queda pendiente salida en vivo durante una operación SteamCMD activa).
...
- [-] Rediseño de navegación tipo sidebar (Fase 1 completa: shell con sidebar persistente + páginas Overview fusionado con Servers, Clusters, Backups, SteamCMD, Logs, Settings; iconografía y responsive con colapso de sidebar a solo-iconos en <900px). Fase "corrección de fidelidad" completa: iconografía migrada a librería real `@phosphor-icons/react` (reemplaza SVGs a mano, mismo `IconName` API), ServerCard rediseñado con thumbnail placeholder, meta-grid de 2 filas (Jugadores/Mapa/Cluster, Mods/Versión/Estado) y fila de acciones icon-only (Iniciar, Detener, Reiniciar, Abrir carpeta, menu kebab con Editar/INI/Logs/Instalar-Actualizar/Clonar/Forzar cierre/Eliminar), tarjeta de estadística de SteamCMD eliminada de Overview y reemplazada por Backups (placeholder) y Updates (real, comparando `officialVersion` vs versión local detectada), "Official Version" reubicado al Sidebar. Fase 2 (rediseño de Logs) completa en su parte estática: tabs superiores Events/Runtime/Update Logs/Backups, Update History paginado con status pills (`success`/`failed`/`unknown` derivado de `exitCode` persistido en el log), Update Details con grid Date/Duration/Size/Type y visor de log coloreado, botón "Open in external viewer" (nuevo IPC `logs:open-update-file` vía `shell.openPath`). Inicio de rewrite UI v2 en `ux_refactor`: base del renderer migrada a Mantine + CSS Modules + aliases nuevos en `electron.vite.config.ts`/`tsconfig.json`; shell nuevo (`AppShellLayout`, `Sidebar`, `PageScaffold`) ya activo y recuperó banner global de errores; `Overview` reimplementado con componentes nuevos (`OverviewPage`, `OverviewStats`, `ServerGrid`, `RecentActivityPanel`), `ServerCard` y `ServerForm` nuevos ya conectados; `SteamCMD` ya migrado como página funcional nueva (`SteamCmdPage` con acciones de instalar, elegir ruta, cancelar y consola); `Clusters`, `Backups`, `Logs` y `Settings` siguen como placeholders homogéneos dentro del shell nuevo; el árbol visual legacy del Overview/SteamCMD ya fue eliminado (`styles.css`, `Icon.tsx`, `Sidebar.tsx`, `ServerCard.tsx`, `ServerForm.tsx`, `LogsViewer.tsx`, `IniEditor.tsx` viejos removidos). Pendiente inmediato: migrar la siguiente página real y reemplazar el placeholder temporal del INI editor cuando llegue su diseño dedicado.
