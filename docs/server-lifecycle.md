...
- Native console and piped stdout remain separate **windows**. Runtime always
  ...
  from **this** run when CFCore/Fatal lines are present.   Closing the native
  console (or Task Manager End task / Ctrl+C) while the server is already
  `running` uses known Windows operator-closed exit codes (`0xC000013A`,
  `0x40010004`, `0x40010005`) and is treated as a clean stop: status becomes
  `stopped` with a **Closed by user** warning on the card, Runtime logs the
  same notice, and no crash toast / `server_crashed` event fires (#524). The
  same codes during `starting` remain unexpected.
...
- **Close window to tray** (default on): the window close button hides YARK to
  ...
  **Hide to tray**, default on; also requires the master switch);
  ...
  sidebar footer **Quit YARK** control (#532; What's-new-style confirm, then the
  same quit path) to exit. There is no native application menu bar (Electron
  default File/Edit/View/Help is disabled).
