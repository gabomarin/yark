...
- Native console and piped stdout remain separate **windows**. Runtime always
  ...
  `0x40010004`, `0x40010005`) and is treated as a clean stop: Runtime logs
  “Server window was closed by the operator”, status becomes `stopped`, and no
  crash toast / `server_crashed` event fires (#524). The same codes during
  `starting` remain unexpected.
