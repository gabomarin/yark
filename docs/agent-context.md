...
- Server profiles have a persisted `enabled` state separate from runtime status. Disabled profiles stay editable and visible through inactive navigation, but the shared backend start path cannot spawn them. Enable/disable is an explicit locked IPC operation; clones inherit the source state and use a unique install directory.
...
- **Move installation** (#56) same-volume rename (verify + rollback) or cross-volume robocopy (Saved included) with free-space progress, then commit `install_dir` and delete the previous folder. Staging leftovers are registered for startup sweep across volumes. Profile `update` must not change `installDir`. IPC: `servers:move-install`, `servers:move-install-cancel`, `servers:move-install-cleanup`, `push:move-install-progress`. E2E: `npm run e2e:move-install`.
