...
| Page / tool panes | Fused chrome shell (`--app-color-surface-chrome`) for tool pages and the server workspace body | Wrapping Logs or the whole form in one rounded Card |
...
**Square vs rounded:** Overview server **list rows** (`ServerCard`), create/edit **form sections**, Logs event/backup/update lists, and the Clusters **How transfers work** Accordion are square (`radius={0}`) so stacked chrome reads as one slab. **Tool panes** (Settings, Clusters, Logs, Backups, Downloads, Hosted Resources): `PageScaffold edgeToEdge` + chrome shell fill (`--app-color-surface-chrome`); page title/actions live **inside** the shell (not a floating header above a card). **Server workspace body** is the same grammar without `PageScaffold`: list rail + main + side rail all chrome, and every full-pane tab shell (server form, mods, launch, INI, backups, logs, RCON, maintenance, ASA API, wizard, onboarding) is square (`radius={0}`) so the tabs read as the pane, not a card floating on canvas. Fused splits (Settings categories, Clusters list+detail) use `gap: 0` and a 1px divider. Raised **panel** cards/rows sit on chrome. Discrete entity cards use `AppSurfaceCard` default **`md`** (8px Comfortable); Overview may stay on the page canvas as the soft home surface. Do not wrap the whole page canvas in an extra Card (#469).
