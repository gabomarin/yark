...
**UI density preference** (Settings → Appearance → Display size):
...
**Theme registry** (`src/renderer/src/shared/theme/themes.ts`, #PUX-004 Track B). A theme is a **palette**, not a second design system: the registry entry carries `{ id, label, colorScheme, palette }`, `theme.ts` reads that palette for the `--ark-*` steps and the Mantine `blue` / `dark` scales, and every `--app-*` role, semantic colour, radius, spacing and motion token stays shared. Adding a theme is one entry plus its palette — feature CSS never gains a second colour language. Preference key: SQLite `app_settings.appearance.v1` (IPC `app:get-appearance` / `app:set-appearance`), JSON `{ "theme": "dark" }`, loaded in `main.tsx` before the first mount like density; an unknown or missing id falls back to the default theme (`parseAppearanceSettings`). **Dark is the only shipped theme** — a light or second dark variant is a registry consumer, not a rewrite.

**Layout profiles** (`src/shared/layout/layoutProfiles.ts`, #PUX-004 Track B). The Adaptive-workspace rules are data, not a `matchMedia` buried in one component: a profile names the viewport width at which the server workspace shows three columns — `adaptive` (1600px, the shipped default) and `drawers` (never; the panes stay in drawers). `workspaceModeFor(profile, width)` is the pure decision, `drawersMediaQuery(profile)` is what the workspace subscribes to, and a test keeps the two in step. CSS cannot read the profile, so the one hand-written mirror — Overview's `@media (min-width: 1600px)` side-by-side — is pinned by that same test.
