...
**Theme registry** (`src/renderer/src/shared/theme/themes.ts`, #PUX-004 Track B). A theme is a **payload**, not a second design system: an entry carries everything that genuinely differs — the palette (ramps, canvas, contrasts, alpha ramps), the semantic colours (`ok` / `attention` / `bad` / `dangerBright` / `fossil`…), the elevation ladder and the Mantine colour scales built from those semantics. `theme.ts` reads all of it; the `--app-*` role map, the density tokens and the motion tokens stay shared, so feature CSS keeps reading roles and never gains a second colour language. **Two themes ship: `dark` (default) and `light`.** Preference key: SQLite `app_settings.appearance.v1` (IPC `app:get-appearance` / `app:set-appearance`), JSON `{ "theme": "dark", "panels": "auto" }`, loaded in `main.tsx` before the first mount like density; an unknown or missing id falls back to the default theme (`parseAppearanceSettings`).
...
- Alphas are tuned per theme: the dark theme uses deep black alphas and a white top highlight, the light theme lighter alphas and a dark hairline (`theme.shadows`).
