...
{backupRoot}/
  World/
    {ISO}-{type}-world-{serverSlug}.zip
  Player profiles/
    {ISO}-{type}-players[-{playerSlug}]-{serverSlug}.zip
  INI/
    {ISO}-{type}-ini-{serverSlug}.zip
```

`{ISO}` is `toISOString()` with `:` / `.` replaced by `-`. `{type}` is the backup
type (`manual`, `scheduled`, `pre_update`, `player_connect`, …). Per-player
session archives include a truncated player slug (max 24).
...
1. **Interrupted `running` rows** — if a readable backup-layout ZIP exists,
   ...
   / non-backup / missing zips are removed when present, then marked `failed`.
   Live creates in `creatingBackupIds` are left alone.
2. **Prune missing paths** — drop completed rows whose archive path is gone;
   keep `failed` history; skip `running`.
3. **Orphan import** — import ZIPs that pass `zipHasBackupLayout` (manifest or
   known roots) and legacy folder archives from kind dirs and the legacy flat
   root. Reuse `manifest.json` backup `id` when free; **mint a new id** when
   that id is already in the DB (copied archives must not collide).

Reconcile changes emit `push:backups-changed`. Destination is reachable when the
backup root **or its parent** exists (`isBackupDestinationReachable`).
...
| --- | --- | --- |
| `backups:list` | `serverId`, `limit?` (handler default **50**; service clamps **1–200**) | `BackupRecord[]` |
...
| `backups:open-folder` | `serverId`, `backupId` | `void` (for ZIP paths, opens the kind directory via `dirname`) |
| `backups:open-root` | `serverId` | `void` (`mkdir` + open root) |
...
- Push: `push:backups-changed` (`BackupsChangedPush` with `serverId`). Emitted on
  create success/fail, delete, restore, retention prune, cleanup, and reconcile
  imports/fixes — **not** on policy-only saves.
...
5. Retention failure after a successful zip is non-fatal (warning event only).

### Restore

1. Backup must be `completed`; server must **not** be active (`ProcessManager.isActive`).
2. UI also blocks while a SteamCMD/files job is busy (`opsLocked`); backend only checks process active.
3. A same-kind `pre_restore` safeguard backup is created first.
4. ZIP archives extract to a temp staging dir; legacy folders are used in place.
5. Apply:
...
6. Restore history is written to SQLite; there is **no** list IPC/UI for it yet.
...
- Every cycle applies retention for each server (**even when schedule is disabled**).
...
| Health `warning` (other) | Any **non-world** failures in 24h, or stale |
| Health `unknown` | Schedule off and no world backup |

Fleet stats: `protectedCount` (`health === "ok"`), `atRiskCount` (warning|critical).

Disk alerts use settings key `backupDiskAlerts.v1` (defaults: warn 85% / critical 95% / free under 20 GiB). Normalized clamps: warn **50–99%**, critical ≥ warn+1 (≤100), free floor **1 GiB**.

Cleanup (`previewCleanup` / `runCleanup`) options: `serverIds`, `includeFailed`,
`enforceRetention`, `olderThanDays`, `keepLastPerKind`, `protectNewestWorld`
(default true), `confirmedBackupIds`. At least one rule is required. `runCleanup`
recomputes the plan, then intersects `confirmedBackupIds`. Cannot delete `running`.
...
- Session packaging also calls `flushWorldIfActive` before create.
...
- On reload, queued jobs reset to `pending`; duplicate pending jobs coalesce waiters.
- Pre-update creates **world + players + ini** (`CRITICAL_BACKUP_KINDS`).
- Safe update stops the server **if it was running**, then takes this snapshot —
...
| Restore rejected | Server still active, files job busy (UI), or backup not `completed` | Stop the server / wait for SteamCMD; only completed backups restore |
