...
- **Server Workspace → Backups** — kind subtabs (**World save** | **Player profiles** | **INI**). Embedded chrome includes a short header with primary **Backup** (world/INI), an `AppMetricCard` strip (last backup / keep last / destination from existing list+policy data), and policy disclosure open by default (collapsible; collapse is not kept across kind changes). World/INI support create + import; Players is join/leave history (export/delete/restore) with no manual create or import.
...
- Visual protocol for the workspace tab: `scripts/visual-backups.cjs` (asserts header/metrics, policy open by default, HD list height).
