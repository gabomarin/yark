...
- Retention keeps the last N **completed** world backups **per map token** (and
  players by `playersRetentionKey`). Fleet **Cleanup** “keep limit” and
  “Keep only last N” use the same per-map / per-player pools (not a flat world
  list). Failed rows are not pruned by retain counts.
...
- **Server Workspace → Backups** — kind subtabs (**World save** | **Player profiles** | **INI**). Embedded chrome includes a short header, a shared **Destination** `PathField` (default root for all three kinds), and per-kind policy disclosure open by default (collapsible; collapse persists within a kind tab but resets when switching kinds). World/INI support **Backup now** + import from the history toolbar; Players is join/leave history (export/delete/restore) with no manual create or import.
