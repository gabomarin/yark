...
| Finish-time helpers | `backupFinishedAt` / `pickLatestFinishedBackup` in the service |
...
3. **Orphan import** — import `.zip` and legacy folder archives from kind dirs
   and the legacy flat root. Orphan **ZIPs** must pass `zipHasBackupLayout`
   (unrelated zips are skipped). Reuse `manifest.json` backup `id` when free;
   **mint a new id** when that id is already in the DB (copied archives must
   not collide).

Fleet “latest” and history ordering use **finish time**
(`completedAt` / `backupFinishedAt`), not job start — which is why mtime
recovery matters for retention and cleanup.
...
5. Retention prune runs after a successful create; a prune failure does **not**
   fail or delete the new archive.

### Restore

1. Backup must be `completed`; backend rejects when the process is active
   (`ProcessManager.isActive`).
2. UI also locks restore while `opsLocked` (SteamCMD / files job busy), even if
   the process has already stopped — backend still only checks `isActive`.
...
- Creates only when `enabled`, interval elapsed since latest **completed world**
  backup finish time (or none yet), and process is active.
- Creates **world only**.
- Retention keeps the last N **completed** backups per kind; players are split by
  `playersRetentionKey`. Failed rows are not pruned by retain counts. Cannot
...
(`protectNewestWorld` default **true**). `runCleanup` always recomputes the plan,
then intersects `confirmedBackupIds`; cannot delete `running`. Player
`keepLastPerKind` pools are per-player.
...
History / “latest” sorting uses `backupFinishedAt`.
...
- Key: `backupCriticalJobsQueue.v1` in app settings (separate from the SteamCMD
  files queue `criticalJobsQueue.v1`).
...
- Safe update stops the server **before** this snapshot — see
  [updates-steamcmd.md](updates-steamcmd.md).

## UI surfaces

- **Sidebar → Backups** — cross-server health, schedule / destination / retention, disk alerts, cleanup; “Open in server” jumps to the workspace tab (often Logs → Backups via `logsFocus`).
...
| Restore rejected / locked | Server still active, files job busy (`opsLocked`), or backup not `completed` | Stop the server; wait for SteamCMD; only completed backups restore |
| Empty ZIP restore hangs | Listeners must be registered before `readEntry` | `extractZip` in `backup-archive.ts` |
| Random `.zip` not imported | Failed `zipHasBackupLayout` | Archive needs `manifest.json` or known layout roots |
| Copied archive missing / wrong id | Manifest id already in DB | Reconcile mints a new id when the manifest id is taken |
| Stuck `running` after crash | Interrupted reconcile | Readable layout zip → completed (mtime); else failed + partial zip deleted |
| Retention not shrinking | Failed / running rows | Only **completed** backups count toward retain N |
| New backup kept but old ones remain | Retention prune error after create | Create still succeeds; check events / disk permissions |
