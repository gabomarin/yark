...
Catalog coverage includes `update_*`, `backup_created` / `backup_deleted` /
`backup_restored`, `server_started` / `server_stopped` / `server_crashed`,
`error`, `rcon_command`, plus a default (other `server_*` types such as created
fall through to the default). Safe-update / verify paths in `UpdateService` pass
...
consume so it cannot stick to another server. Overview opens
`{ section: "updates" }` only when opening a server whose SteamCMD job is
currently busy; Backups fleet can open `{ section: "backups" }`. When `eventId`
is set, the panel forces the Events section (event wins over a file section).
...
section’s clear/delete uses `backups:delete` (not a `logs:*` channel).
