...
2. **Running:** arm ServerChat window from `updateWarnings`, then at T0
   `UpdateService.enqueueUpdateForMaintenance` — sets `wasRunning`, **waits** for
   stop `{ backup: false }` → `pre_update` world+ini → SteamCMD → start (or rollback).
   See [updates-steamcmd.md](updates-steamcmd.md).
3. **Stopped:** start `updateServer` **without awaiting inside the policy loop**
   (completion still updates `lastUpdate*` off-loop).
4. Does not overlap a restart countdown on the same server.
5. Fail-streak pause (`MAINTENANCE_FAIL_LIMIT`) with Resume; failed Steam builds also
   cool down 5 minutes before re-arm. Availability key = `${serverId}:${officialBuild}`.
6. **Run update now** requires `updateEnabled` + running + Steam newer; stopped servers
