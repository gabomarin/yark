...
## Follow-up: `runWithFinally` (#404)

**Date:** 2026-08-22  
**Helper:** `src/renderer/src/shared/async/runWithFinally.ts`

The spike’s top skip (`try`/`finally` without `catch` in component bodies) is
addressed by moving async cleanup into a module-level helper so
`babel-plugin-react-compiler` can optimize the component.
