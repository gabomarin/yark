...
| ------------------- | ---------------------------------------------------- | ---------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Atom**            | Tiny presentational unit; little/no feature logic    | `src/renderer/src/shared/ui/*`                       | `AppSurfaceCard`, `EmptyState`, `SearchField`, `ReadonlyPath`, `PathField`, `ConsoleSurface`, `AppMetricCard`, `ServerRuntimeStatusBadge`, `StatusWord`, `DismissibleHint`, `SelectableListRow`, `NavSelected` (`navSelectedClassName` on Mantine NavLink), `YarkDataTable`, `AccentIconTile` |
...
| ASA / domain tokens                        | Named export in the domain module (`shared/asa/map-token-suggest`, `structured-launch-options`, Steam app id).                                                                                                                                                                                                                                                  | A regex or suffix used in **one** function with a clear local name — do not constant-wrap every unique pattern.  |
