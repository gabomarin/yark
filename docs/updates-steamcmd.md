...
| --- | --- | --- |
| Install files | SidePanel (and similar) asks for a stopped server (“Stop the server before installing base files”); Overview does not hard-disable Install; backend has no auto-stop | Leaves process state alone |
...
UI entry points: sidebar **SteamCMD** page + floating progress dock; Overview
install/update/verify; workspace SidePanel; onboarding “Install files”.
Update/verify stay enabled while the server is running (SidePanel tooltips warn
about stop/restart). Install is locked on SidePanel while active; Overview does
not hard-disable Install.

Opening a server workspace **while that server’s files job is busy** focuses
Logs → Updates (`initialTab: "logs"`, `logsFocus: { section: "updates" }`).

## Progress

Live progress combines SteamCMD stdout `%` lines, install `appmanifest`
`BytesDownloaded`, tails of `logs/console_log.txt`, and disk estimates under
`downloading/` (`steamcmd-disk-progress.ts`). The dock and SteamCMD page
subscribe to `push:steamcmd-progress` (throttle `PROGRESS_PUSH_MIN_MS = 100`).
