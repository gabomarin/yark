...
**Maintenance exception (#489):** workspace Maintenance auto-update / Run update now
calls `UpdateService.enqueueUpdateForMaintenance(serverId)`. That path may start
while the dedicated is running: it sets `wasRunning`, waits for stop `{ backup: false }`
→ `pre_update` world+ini → SteamCMD → start (or rollback). Stopped-server auto-update
still uses the normal stopped `updateServer` path. Full job model:
...
| `enqueueUpdateForMaintenance` (internal) | Maintenance-only; may stop a running server then run the same safe-update pipeline with `wasRunning` |
