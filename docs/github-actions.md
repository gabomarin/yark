...
| [`.github/workflows/discord-release-notify.yml`](../.github/workflows/discord-release-notify.yml) | Post release + changelog body to Discord `#releases` webhook | `contents: read` |
...
After each published GitHub Release (including `0.x` prereleases), a follow-up
workflow posts the tag, download links, and release notes body to Discord.
...
3. Optional dry-run: Actions → **Discord release notify** → Run workflow → enter an
   existing tag (e.g. `v0.19.0`).

The job uses `curl`-equivalent Python only (no third-party Discord Action). Message
`flags` include `SUPPRESS_EMBEDS` so GitHub links do not expand into preview cards.
Long release notes are truncated to Discord’s 2000-character limit with a pointer
to the full GitHub release page.
