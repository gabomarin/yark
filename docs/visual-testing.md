...
| `node scripts/visual-plasma-breeze.cjs`          | Plasma Breeze pass (#PUX-005-B): for Breeze Dark and Breeze Light, asserts the exact `#3daee9` accent and the Noto Sans typography mounted, then captures Overview / Settings / Appearance / the New server overlay at HD / Full HD. Workspace tabs run with `YARK_VISUAL_FAMILY=plasma-breeze node scripts/visual-workspace-tabs.cjs`. |
