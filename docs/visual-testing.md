...
workspace, review at least the Server, Launch, Mods, `INI Files` (`Game.ini` /
`GameUserSettings.ini`), Backups, and Logs tabs.
...
| `node scripts/visual-workspace-tabs.cjs`         | Workspace tab coverage: walks all nine tabs (Server, INI Files, Mods, Launch, Backups, Logs, RCON, Maintenance, Ark Server API) at HD / Full HD. Asserts the shell content box does not overflow and nothing is covered by the SteamCMD footer - the two shapes of the "view sized against the viewport" bug. `YARK_VISUAL_THEME=light` runs the same walk on the light theme (no grain, so elevation and hairlines carry the separation); `YARK_VISUAL_DENSITY=comfortable` runs it in the other display size (Compact is the product default, so it is what a plain run captures). |
| `node scripts/visual-hosted-resources.cjs`       | Hosted Resources page at HD / Full HD: status card, diagnostics panel, and the same content-box / footer-overlap contract. Deliberately does not touch the enable toggle, because switching it on binds a listener.                                                                                                                                                                                                                                                                                                                                                                  |
...
| `node scripts/visual-light-theme.cjs`            | Light theme pass: seeds `appearance.v1`, asserts the mounted scheme, captures Overview / Settings / Appearance at HD / Full HD / QHD                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `node scripts/visual-plasma-breeze.cjs`          | Plasma Breeze pass (#PUX-005-B): for Breeze Dark and Breeze Light, asserts the exact `#3daee9` accent and Noto Sans typography, captures Overview / Settings / Appearance / sidebar routes / New server overlay at HD / Full HD / QHD, and the setup assistant welcome overlay at QHD. Workspace tabs run with `YARK_VISUAL_FAMILY=plasma-breeze node scripts/visual-workspace-tabs.cjs`.                                                                                                                                                                                            |
...
| `visual-downloads.cjs`            | `npm run build && node scripts/visual-downloads.cjs`                                                   | Two isolated launches (#201). **Needs attention** (no SteamCMD): boot-blocked pending, cancelled, failed retry/dismiss, missing profile, crash/ambiguous, then operator cancel of Paused; Advanced log auto-opens. **Happy path** (hanging SteamCMD stub): pending leftovers resume to Active on launch; Queued/Paused stay live; SteamCMD bar only on the active job; Advanced log stays collapsed with a one-line status; Install replaces queued Verify with no leftover attention; Cancel download / Cancel SteamCMD _do_ move those rows to Needs attention. HD / Full HD / QHD. |
...
| `e2e:smoke` / `e2e`               | `npm run build && npm run e2e:smoke` / `npm run e2e`                                                   | `e2e` is **PR CI** on `windows-latest` (#12); `e2e:smoke` is local empty-fleet. Isolated `YARK_E2E_USER_DATA`; clears `ELECTRON_RUN_AS_NODE`. Matrix: [e2e-validation.md](e2e-validation.md).                                                                                                                                                                                                                                                                                                                                                                                         |
