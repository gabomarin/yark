...
| `seed-server-logs.cjs` | `node scripts/seed-server-logs.cjs [serverName]` | Seeds events + update log files for Logs UI review. Resolves Electron userData cross-platform (`%APPDATA%` / macOS Application Support / `~/.config`); override with `YARK_USER_DATA`. See [logs.md](logs.md). |
...
Related domain context: [updates-steamcmd.md](updates-steamcmd.md), [logs.md](logs.md), [AGENTS.md](../AGENTS.md).
