...
| `node scripts/visual-workspace-tabs.cjs`         | Workspace tab coverage: walks all nine tabs (Server, INI Files, Mods, Launch, Backups, Logs, RCON, Maintenance, Ark Server API) at HD / Full HD. Asserts the shell content box does not overflow and nothing is covered by the SteamCMD footer - the two shapes of the "view sized against the viewport" bug. |
