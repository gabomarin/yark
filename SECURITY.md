...
- **Local Windows account access** reading ASA INI files, INI backup ZIPs, or the local SQLite profile (which may contain plaintext passwords and an optional Discord webhook token), or another process already running as the same user — documented residual risk ([#144](https://github.com/gabomarin/yark/issues/144), [docs/credential-threat-model.md](docs/credential-threat-model.md))
- **Operator-configured Discord webhook delivery** — YARK accepts only official `https://discord.com/api/webhooks/...` URLs, sends outbound event embeds with mentions disabled, and never exposes remote control commands. Revoke a webhook in Discord if its URL is disclosed.
...
- **Automatic public-IP lookup** for the workspace (`app:get-public-ip`) — the main process calls `ipify.org` over HTTPS when a server starts, when **Share connection details** opens without a detected address, or when the operator refreshes it. The lookup returns only the detected IPv4; revealing the public IP to that service is inherent to the lookup. It is best-effort, and the address is read-only in the dialog.
...
- **Experimental Hosted Resources loopback host** ([#564](https://github.com/gabomarin/yark/issues/564), default off) — binds `127.0.0.1` only, serves the published immutable revision of **enabled** resources by opaque token over `GET`/`HEAD`, fails closed when the port is busy or foreign-owned, and is not a LAN/WAN/HTTPS server. Disabling stops serving immediately (including after restart) and is reversible; deleting removes the resource and its revisions. It is a local, same-machine capability; the URL is disclosed to whatever can read the INI, and it stops working when YARK exits ([docs/hosted-resources.md](docs/hosted-resources.md))
