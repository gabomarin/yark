...
import type { ServerProfile, ServerStatus, SessionPortSet, StartServerOptions } from "@shared/types";
import { resolveAsaApiInjectMode, syncAsaApiVersionDll } from "../../domains/asa-api/asa-api-inject";
import {
  buildLaunchArgs,
...
import { AsaSavedLogsTailer, captureAsaLogSessionAnchor, type AsaLogSessionAnchor } from "./asa-log-tail";
...
export interface ProcessStartHost {
  ...
  captureRuntimeChunk(serverId: string, source: RuntimeLogSource, chunk: string): void;
  emitStatus(serverId: string): void;
  writeProcessCheckpoint(serverId: string, managed: ProcessStartManaged): Promise<void>;
  waitUntilReady(profile: ServerProfile, managed: ProcessStartManaged, generation: number): Promise<void>;
  flushRuntimePartials(serverId: string): void;
  clearProcessCheckpoint(serverId: string): void;
  onManagedExit(serverId: string, managed: ProcessStartManaged, code: number | null): void;
}
...
async function adoptAsaGameChild(
  ...
  if (loaderPid === undefined || loaderPid <= 0) {
    host.appendRuntimeLog(profile.id, "warning", "AsaApiLoader started without a PID; continuing with loader handle");
    return;
  }
  managed.loaderPid = loaderPid;
  host.appendRuntimeLog(profile.id, "system", `AsaApiLoader pid ${loaderPid}; waiting for ArkAscendedServer.exe child`);
  ...
  for (let attempt = 0; attempt < ASA_CHILD_ADOPT_ATTEMPTS; attempt += 1) {
    ...
    const gamePid = await findWindowsChildProcessByName(loaderPid, "ArkAscendedServer.exe");
    ...
    host.appendRuntimeLog(profile.id, "system", `Adopted ArkAscendedServer.exe pid ${gamePid} (loader ${loaderPid})`);
...
export function startManagedProcess(
  ...
  if (viaLoader) {
    if (!existsSync(binary)) {
      throw new Error(`AsaApiLoader.exe not found at: ${binary}. Install AsaApi from the AsaApi tab first.`);
    }
  ...
  if (injectMode === "versionDll") {
    host.appendRuntimeLog(profile.id, "system", "AsaApi Version.dll mode — starting ArkAscendedServer.exe directly");
  } else if (viaLoader) {
    host.appendRuntimeLog(profile.id, "system", `AsaApiLoader mode — game binary ${gameBinary}`);
  }
  if (asaApiLoading) {
    host.appendRuntimeLog(
      profile.id,
  ...
  child.once("spawn", () => {
    if (host.getManaged(profile.id) !== managed || managed.status !== "starting") {
      return;
    }
    host.appendRuntimeLog(profile.id, "system", "Process created; waiting for server readiness (RCON / startup)");
    void host.writeProcessCheckpoint(profile.id, managed);
    host.emitStatus(profile.id);
    ...
    if (options?.skipReadinessCheck === true) {
      ...
      host.appendRuntimeLog(profile.id, "system", "Readiness skipped (skipReadinessCheck); status running");
      host.emitStatus(profile.id);
      return;
    }
