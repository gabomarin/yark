...
import { FLEET_INSPECT_CONCURRENCY, mapPool } from "./instance-lifecycle";
import { buildFleetInspectKey, fleetServerSetChanged, shouldInspectFleetInstallations } from "./instance-profile";
...
export class InstanceFleetInstall {
  ...
  clearServer(serverId: string): void {
    this.lastInstallServers = this.lastInstallServers.filter((info) => info.serverId !== serverId);
  ...
  async installationInfo(
    ...
    const officialChanged =
      this.lastOfficialVersion !== officialVersion || this.lastOfficialSteamBuild !== officialSteamBuild;
   ...
   */
  async inspectFleetInstallations(options: { bypassCache: boolean }): Promise<ServerInstallationInfo[]> {
    for (;;) {
      const profiles = this.deps.repo.list();
  ...
  async runFleetInstallScan(
    ...
    return mapPool(profilesToScan, FLEET_INSPECT_CONCURRENCY, async (profile) => {
      ...
      if (info.health === "ready" && resolveDisplayedServerVersion(info) == null) {
  ...
  recordInstallHealth(info: ServerInstallationInfo): void {
    ...
    this.deps.repo.addEvent(
      ...
      info.health === "inaccessible" || info.health === "suspicious" ? "error" : "warning",
