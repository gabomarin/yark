 ...
 */
export function reconcileServerList(previous: ServerProfile[], next: ServerProfile[]): ServerProfile[] {
  if (
    previous.length === next.length &&
    previous.every((server, index) => {
      const candidate = next[index];
      return candidate !== undefined && server.id === candidate.id && server.updatedAt === candidate.updatedAt;
    })
  ) {
    return previous;
  }
  ...
  return next.map((server) => {
    ...
    return prior !== undefined && prior.updatedAt === server.updatedAt ? prior : server;
  });
}
