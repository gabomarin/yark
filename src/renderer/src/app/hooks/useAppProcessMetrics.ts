...
function sameMetrics(left: ProcessMetricsUpdatedPush | undefined, right: ProcessMetricsUpdatedPush): boolean {
...
function upsertProcessMetrics(
  ...
  if (sample.pid <= 0 && sample.workingSetBytes == null && sample.cpuPercent == null) {
    ...
    const unsubscribe =
      typeof window.api.onProcessMetricsUpdated === "function"
        ? window.api.onProcessMetricsUpdated((payload) => {
            setProcessMetricsByServer((prev) => upsertProcessMetrics(prev, payload));
          })
        : () => undefined;
    return () => {
      unsubscribe();
