...
export function useAppFleetRefresh(options: { route: Route; overlay: Overlay | null }): {
  ...
  const [installationInfo, setInstallationInfo] = useState<Map<string, ServerInstallationInfo>>(new Map());
  const [officialVersion, setOfficialVersion] = useState<string | null>(null);
  const [officialNetworkStatus, setOfficialNetworkStatus] = useState<OfficialNetworkStatus>("unknown");
  ...
  const [stopProgressByServerId, setStopProgressByServerId] = useState<Map<string, ServerStopProgress>>(new Map());
  ...
  const refresh = useCallback(
    async (refreshOptions?: {
      ...
      const [serversRes, statusesRes, installRes, steamCmdRes, steamCmdConsoleRes, clusterRes, eventsRes] =
        await Promise.all([
          includeServerList ? window.api.listServers() : Promise.resolve(null),
          window.api.getStatuses(),
          includeInstallation ? window.api.getInstallationInfo(forceOfficialCheck, serversMode) : Promise.resolve(null),
          window.api.getSteamCmdStatus(),
          window.api.getSteamCmdConsole(140),
          window.api.checkCluster(),
          window.api.recentEvents(100),
        ]);
      const decision = decideRefreshGeneration(generation, refreshGenerationGateRef.current, isAction);
      if (decision.commit) {
        if (serversRes !== null && serversRes.ok) {
          setServers((previous) => reconcileServerList(previous, serversRes.data));
        }
        if (statusesRes.ok) {
          setStatuses((previous) => reconcileStatusMap(previous, statusesRes.data));
        }
        if (installRes !== null && installRes.ok) {
          setOfficialVersion((previous) =>
            previous === installRes.data.officialVersion ? previous : installRes.data.officialVersion,
          );
          setOfficialNetworkStatus((previous) =>
            previous === installRes.data.officialNetworkStatus ? previous : installRes.data.officialNetworkStatus,
          );
          setOfficialSteamBuild((previous) =>
            previous === installRes.data.officialSteamBuild ? previous : installRes.data.officialSteamBuild,
          );
          setInstallationInfo((previous) => reconcileInstallationMap(previous, installRes.data.servers));
        }
        if (steamCmdRes.ok) {
          setSteamCmdStatus((previous) => reconcileSteamCmdStatus(previous, steamCmdRes.data));
        }
        if (steamCmdConsoleRes.ok) {
          setSteamCmdConsole((previous) => reconcileSteamCmdConsole(previous, steamCmdConsoleRes.data));
        }

        if (clusterRes.ok) {
          setReports((previous) => reconcileClusterReports(previous, clusterRes.data));
        }
        if (eventsRes.ok) {
      ...
      return {
        servers: serversRes !== null && serversRes.ok ? serversRes.data : null,
        statuses: statusesRes.ok ? new Map(statusesRes.data.map((s) => [s.serverId, s])) : null,
        installationInfo:
          installRes !== null && installRes.ok ? new Map(installRes.data.servers.map((s) => [s.serverId, s])) : null,
        officialSteamBuild: installRes !== null && installRes.ok ? installRes.data.officialSteamBuild : null,
      };
    },
    [],
  );
    ...
    async (reason: "startup" | "manual") => {
      ...
      const job = runWithFinally(
        async () => {
          ...
          if (snapshot.servers === null || snapshot.statuses === null || snapshot.installationInfo === null) {
            showOperatorError("Try Check Servers Health again in a moment.", "Could not finish health check");
            return;
          }
          ...
          if (issues.length > 0) {
            showOperatorToast({
              title: issues.length === 1 ? "1 server needs attention" : `${issues.length} servers need attention`,
          ...
          const unverifiedInstalls = [...snapshot.installationInfo.values()].filter(
            (info) => info.installed && getServerUpdateState(info, snapshot.officialSteamBuild) === "unknown",
          ).length;
          if (unverifiedInstalls > 0) {
            showOperatorToast({
              title: "Installs look OK; updates unverified",
              message: "Couldn't confirm Steam update status for every server. Try Check server updates.",
              color: "attention",
            });
            return;
          }
  ...
  useEffect(() => {
    ...
    const unsubscribeProgress = window.api.onSteamCmdProgress((payload) => {
      setSteamCmdStatus((previous) => reconcileSteamCmdStatus(previous, payload.status));
      setSteamCmdConsole((previous) => reconcileSteamCmdConsole(previous, payload.console));
    });
  ...
  useEffect(() => {
    const shouldPollSteamCmd = overlay === null && (route === "overview" || route === "downloads");
  ...
  }, [refresh, steamCmdBusy, steamCmdStatus?.operation, route, overlay]);
