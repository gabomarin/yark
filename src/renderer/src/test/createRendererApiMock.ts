...
export function createRendererApiMock(
  ...
  return {
    ...
    setAdminList: vi.fn().mockResolvedValue({
      ok: true,
      data: {
        mode: "local",
        adminListUrl: "",
    ...
    validateAdminListUrl: vi.fn().mockResolvedValue({
      ok: true,
      data: { count: 0, ids: [] },
    }),
    learnAdminListNames: vi.fn().mockResolvedValue({
      ok: true,
      data: { updated: 0 },
    }),
    ...
    getMaintenancePolicy: vi.fn().mockResolvedValue({
      ok: true,
      data: {
        serverId: "srv-1",
