...
vi.mock("@mantine/carousel", () => {
  function CarouselRoot(props: { children?: React.ReactNode; "aria-label"?: string }) {
...
const baseDetail: ModMetadata = {
  ...
  thumbnailUrl: null,
  screenshots: ["https://cdn.example/shot-a.jpg", "https://cdn.example/shot-b.jpg"],
  authors: ["ArkCartographers"],
  downloadCount: 1000,
  dateModified: "2026-08-12T18:40:00.000Z",
  curseforgeUrl: "https://www.curseforge.com/ark-survival-ascended/mods/scorched",
  slug: "scorched",
  categories: ["Maps"],
};
...
describe("ServerModDetailDrawer #342", () => {
  it("shows description when present", async () => {
    ...
    expect(screen.getByLabelText(/curseforge screenshots/i)).toBeInTheDocument();
    expect(screen.getByText(/long author notes/i)).toBeInTheDocument();
    expect(screen.queryByRole("button", { name: /show more/i })).not.toBeInTheDocument();

    await user.click(screen.getByRole("button", { name: /screenshot 1, enlarge/i }));
  ...
  it("hides screenshot chrome when the list is empty", () => {
    ...
    expect(screen.queryByLabelText(/curseforge screenshots/i)).not.toBeInTheDocument();
  });
