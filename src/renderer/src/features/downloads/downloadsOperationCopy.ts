...
export const FILES_QUEUE_OPERATIONS = new Set<CriticalJobOperation>(["install-files", "update", "verify-files"]);

const OPERATION_TITLE: Record<NonNullable<SteamCmdStatus["operation"]> | CriticalJobOperation, string> = {
...
export function operationTitle(operation: NonNullable<SteamCmdStatus["operation"]> | CriticalJobOperation): string {
  return OPERATION_TITLE[operation];
}

/** Internal backup sub-step of safe update — never a separate Downloads row. */
export function isOperatorVisibleCriticalJob(job: { operation: CriticalJobOperation }): boolean {
