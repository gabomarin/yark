...
export function ServerCardStatusBadges(props: Props): ReactElement {
  const displayStatus =
    props.startBusy && (props.status === "stopped" || props.status === "error") ? "starting" : props.status;
  const asaApiLoading = props.asaApiLoading === true && (displayStatus === "starting" || props.status === "starting");
  const statusLabel = props.stopBusy
    ...
    : asaApiLoading
      ...
      : props.startBusy && (props.status === "stopped" || props.status === "error")
        ...
        : undefined;
  const statusColor =
    props.stopBusy || asaApiLoading || (props.startBusy && (props.status === "stopped" || props.status === "error"))
      ? "blue"
      : undefined;
  ...
  return (
    <div className={classes.statusBadges}>
      <span className={classes.statusLabelSlot}>
        <ServerRuntimeStatusBadge status={displayStatus} label={statusLabel} color={statusColor} appearance="label" />
      </span>
      <span className={classes.statusDotSlot}>
        <ServerRuntimeStatusBadge status={displayStatus} label={statusLabel} color={statusColor} appearance="dot" />
      </span>
      {!props.serverEnabled && (
