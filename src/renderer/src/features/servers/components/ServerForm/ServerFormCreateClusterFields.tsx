...
export function ServerFormCreateClusterFields(props: Props): ReactElement {
  ...
  const selected =
    props.selectedClusterId === null
      ? null
      : (props.options.find((option) => option.clusterId === props.selectedClusterId) ?? null);
      ...
      {!hasClusters &&
        (props.onOpenClusters !== undefined ? (
          <Text size="sm" c="dimmed">
            ...
            <Anchor component="button" type="button" underline="hover" onClick={props.onOpenClusters}>
              Create a cluster first…
            </Anchor>
          </Text>
        ) : (
          <Text size="sm" c="dimmed">
            Create a cluster from the Clusters page, then come back to join it here.
          </Text>
        ))}
      {selected !== null && (
        <>
