...
export function SettingsDiscordSection(): ReactElement {
  const [draft, setDraft] = useState<DiscordWebhookPreferences>(
    DEFAULT_DISCORD_WEBHOOK_PREFERENCES,
  );
  ...
  return (
    <section className={classes.section} aria-labelledby="settings-discord">
      <div>
        <Title order={3} size="h4" id="settings-discord">Discord</Title>
      ...
      <AppSurfaceCard tone="flat" padding="sm" radius={0} className={classes.discordConnectionCard}>
        ...
        <Group
          ...
          mt="sm"
          wrap="nowrap"
          className={classes.discordConnectionFields}
        >
          <PasswordInput
      ...
      <Group justify="space-between" align="flex-end" gap="sm">
        ...
        <Popover width={360} position="bottom-end" withArrow shadow="md">
          ...
          <Popover.Dropdown>
            <Stack gap="xs">
              <Text size="xs">
                <Text component="span" fw={600}>{`{server}`}</Text>
                {" "}inserts the server name. Example: {`{server} crashed`} → Island crashed.
              </Text>
              <Text size="xs">
                <Text component="span" fw={600}>{`{detail}`}</Text>
                {" "}inserts SteamCMD job detail and works only for SteamCMD events.
              </Text>
              <Text size="xs" c="dimmed">
                Discord formatting works: **bold**, *italics*, __underline__, ~~strikethrough~~,
                and `code`. Mentions stay off.
              </Text>
            </Stack>
          </Popover.Dropdown>
        </Popover>
      ...
      <SimpleGrid cols={{ base: 1, sm: 2 }} spacing="md" data-discord-events-grid>
        ...
        >
          <Group gap="xs" className={classes.discordEventCardHeader}>
            <HardDrives size={17} />
            <Text size="sm" fw={600}>Server events</Text>
          </Group>
      ...
      <Group gap="xs" wrap="nowrap" className={classes.discordPrivacyNote}>
        <Info size={14} />
        <Text size="xs" c="dimmed">
          Changes save automatically. Discord receives alerts only and cannot control your servers.
          Delete the webhook in Discord if its URL leaks.
        </Text>
      </Group>
    </section>
  );
