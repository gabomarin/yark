...
Default schedule policy (when enabled): interval 60 minutes; retain ~20 world **per map** / ~20 players / ~10 INI (configurable). Fleet **Cleanup** “keep limit” uses those same per-map / per-player pools. The schedule waits a full interval after the process becomes active, counts failed attempts toward the interval, and can pause for the rest of the YARK session after repeated failures (policy stays enabled). Use **Clear failed** on the history toolbar to drop failed rows that left no archive on disk.
