...
| Stop | Best-effort **`pre_stop`** (world + INI) after graceful exit |
| Restart | Fail-hard **`pre_restart`** (world + INI) after stop, before start |
| Safe update | Fail-hard **`pre_update`** (world + INI) before SteamCMD work |

Default schedule policy (when enabled): interval 60 minutes; retain ~20 world **per map** / ~20 players / ~10 INI (configurable). Fleet **Cleanup** “Delete older backups past each server's keep limit” uses those same per-map / per-player pools. **Keep only last N per kind** is still a flat newest-first list for world and INI (players stay per-player) — prefer the keep-limit rule when rotating maps. The schedule waits a full interval after the process becomes active, counts failed attempts toward the interval, and can pause for the rest of the YARK session after repeated failures (policy stays enabled). Use **Clear failed** on the history toolbar to drop failed rows that left no archive on disk.
