---
title: Edit GameUserSettings.ini with a visual INI editor
description: Edit GameUserSettings.ini and Game.ini for an ASA dedicated server in YARK — visual and raw editors plus the six-step configuration assistant.
---

import { Aside, Steps } from "@astrojs/starlight/components";
import { withBase } from "../../../data/site";

Paths under each install:

```text
{installDir}/ShooterGame/Saved/Config/WindowsServer/
  GameUserSettings.ini
  Game.ini
```

## Visual + raw editors

- Read seeds missing files from built-in defaults and sanitizes client noise (resolution, volume, scalability keys, etc.)
- Save sanitizes → validates a few semantic ranges → writes both files → best-effort INI backup
- Do not expect generic INI libraries to round-trip ASA — section names can contain dots literally

Semantic checks include RCON port range and `DifficultyOffset` 0–1 when `[ServerSettings]` is present, and `MaxPlayers` 1–255 on `[/Script/Engine.GameSession]`. ASA ignores leftover `[ServerSettings] MaxPlayers`; YARK hides that key in the editor and strips it on start/sync so the slot limit follows **Max players** on the Server form.

Profile networking fields (RCON, passwords, session, ports) are also written on **start** via profile→INI sync. See [Start, stop, restart](/docs/lifecycle/).

<figure class="doc-shot">
  <img
    src={withBase("/screenshots/workspace-ini.png")}
    alt="Visual INI editor showing GameUserSettings.ini options with toggles, values, and descriptions"
    width="1440"
    height="900"
    loading="lazy"
  />
  <figcaption>Visual INI editor — searchable controls for GameUserSettings.ini / Game.ini.</figcaption>
</figure>

## Configuration assistant

On-demand from the Server tab — **not** a permanent nav item.

<figure class="doc-shot">
  <img
    src={withBase("/screenshots/configuration-wizard.png")}
    alt="Configuration assistant with a six-step wizard and preset server-type cards"
    width="1440"
    height="900"
    loading="lazy"
  />
  <figcaption>Configuration assistant — guided presets; nothing is written until you apply.</figcaption>
</figure>

<Steps>

1. **Profile** — identity-oriented choices
2. **Pace** — progression rate presets
3. **Breeding**
4. **World**
5. **QoL**
6. **Review** — nothing is written until you apply

</Steps>

Rules that matter:

- Blocked while the manual INI editor has **unsaved dirty** edits
- Draft-only until **Apply changes**
- Apply re-reads latest INI, overlays only curated keys, preserves unknown keys / casing
- Difficulty keys rewrite only when you make an explicit level choice

<Aside type="tip">
  Use the assistant to bootstrap a server type, then fine-tune in the visual INI editor.
  Take a backup before large rate changes on a live world.
</Aside>
